...
# Load hand-coded AfD results data
afd_results <- read_excel('~/Dissertation data analysis/Data/Constituency returns/afd_results.xlsx')

# Clean environment
rm(list = setdiff(ls(), c('d_raw', 'pop_den17', 'pop_den21', 'foreign17', 'foreign21', 'unemp17', 'unemp21', 'afd_results')))
...
# Clean environment
rm(list = setdiff(ls(), c('d', 'd_raw', 'pop_den17', 'pop_den21', 'foreign17', 'foreign21', 'unemp17', 'unemp21', 'afd_results')))
...
### 2017 treatment
# Create 2017 dataset
d17_cont <- d[d$wave == 2 | d$wave == 5 | d$wave == 8, ]
d17_cont$land <- as.character(d17_cont$land)
afd_results17 <- afd_results[afd_results$year == 2017,]

# Merge
d17_cont <- merge(d17_cont, afd_results17, by = 'land')

# Create post variable
d17_cont$post17 <- ifelse(d17_cont$wave == 8, 1, 0)

# Models 2017
summary(lm(bundestag_trust ~ share_seats + post17 + share_seats * post17, data = d17_cont))
summary(lm(swd ~ share_seats + post17 + share_seats * post17, data = d17_cont))

### 2021 treatment
# Create 2021 dataset
d21_cont <- d[d$wave == 16 | d$wave == 20, ]
d21_cont$land <- as.character(d21_cont$land)
afd_results21 <- afd_results[afd_results$year == 2021,]

# Merge
d21_cont <- merge(d21_cont, afd_results21, by = 'land')

# Create post variable
d21_cont$post21 <- ifelse(d21_cont$wave == 20, 1, 0)

# Models 2021
summary(lm(bundestag_trust ~ share_seats + post21 + share_seats * post21, data = d21_cont))
summary(lm(swd ~ share_seats + post21 + share_seats * post21, data = d21_cont))

## HYPOTHESIS 1 
### 2017 treatment
# Subset to 2017 AfD respondents
afd17_cont <- d17_cont %>% 
  group_by(lfdn) %>% 
...
# Models 2017
summary(lm(bundestag_trust ~ share_seats + post17 + share_seats * post17, data = afd17_cont))
summary(lm(swd ~ share_seats + post17 + share_seats * post17, data = afd17_cont))

### 2021 treatment
# Subset to 2021 AfD respondents
afd21_cont <- d21_cont %>% 
  group_by(lfdn) %>% 
  filter(any(first_vote_a == 322 | first_vote_b == 322)) %>% 
  ungroup()
...
# Models 2021
summary(lm(bundestag_trust ~ share_seats + post21 + share_seats * post21, data = afd21_cont))
summary(lm(swd ~ share_seats + post21 + share_seats * post21, data = afd21_cont))
...
# Subset to Saxony
d17sc <- d17[d17$land == 14, ]

# Check size of treatment and control
n_distinct(d17sc %>% filter(treat17 == 1) %>% select(lfdn)) # 112
n_distinct(d17sc %>% filter(treat17 == 0) %>% select(lfdn)) # 439

# Models without time-varying covariates
summary(lm(bundestag_trust ~ treat17 + post17 + treat17 * post17, data = d17sc))
summary(lm(swd ~ treat17 + post17 + treat17 * post17, data = d17sc))

# Models with time-varying covariates
summary(lm(bundestag_trust ~ treat17 + post17 + treat17 * post17 + ln_pop_den + perc_foreign + unemp_rate, data = d17sc))
summary(lm(swd ~ treat17 + post17 + treat17 * post17 + ln_pop_den + perc_foreign + unemp_rate, data = d17sc))
...
# Subset to Saxony, Saxony-Anhalt, and Thuringia
d21sc <- d21[d21$land == 14 | d21$land == 15 | d21$land == 16, ]

# Check size of treatment and control
n_distinct(d21sc %>% filter(treat21 == 1) %>% select(lfdn)) # 581
n_distinct(d21sc %>% filter(treat21 == 0) %>% select(lfdn)) # 994

# Models time-varying covariates
summary(lm(bundestag_trust ~ treat21 + post21 + treat21 * post21, data = d21sc))
summary(lm(swd ~ treat21 + post21 + treat21 * post21, data = d21sc))

# Models with time-varying covariates
summary(lm(bundestag_trust ~ treat21 + post21 + treat21 * post21 + ln_pop_den + perc_foreign + unemp_rate, data = d21sc))
summary(lm(swd ~ treat21 + post21 + treat21 * post21 + ln_pop_den + perc_foreign + unemp_rate, data = d21sc))
...
# Check size of treatment and control
n_distinct(afd17sc %>% filter(treat17 == 1) %>% select(lfdn)) # 22
n_distinct(afd17sc %>% filter(treat17 == 0) %>% select(lfdn)) # 68

# Models without time-varying covariates
summary(lm(bundestag_trust ~ treat17 + post17 + treat17 * post17, data = afd17sc))
summary(lm(swd ~ treat17 + post17 + treat17 * post17, data = afd17sc))

# Models with time-varying covariates
summary(lm(bundestag_trust ~ treat17 + post17 + treat17 * post17 + ln_pop_den + perc_foreign + unemp_rate, data = afd17sc))
summary(lm(swd ~ treat17 + post17 + treat17 * post17 + ln_pop_den + perc_foreign + unemp_rate, data = afd17sc))
...
# Check size of treatment and control
n_distinct(afd21sc %>% filter(treat21 == 1) %>% select(lfdn)) # 109
n_distinct(afd21sc %>% filter(treat21 == 0) %>% select(lfdn)) # 110

## Models without time-varying covariates
summary(lm(bundestag_trust ~ treat21 + post21 + treat21 * post21, data = afd21sc))
summary(lm(swd ~ treat21 + post21 + treat21 * post21, data = afd21sc))

## Models with time-varying covariates
summary(lm(bundestag_trust ~ treat21 + post21 + treat21 * post21 + ln_pop_den + perc_foreign + unemp_rate, data = afd21sc))
summary(lm(swd ~ treat21 + post21 + treat21 * post21 + ln_pop_den + perc_foreign + unemp_rate, data = afd21sc))
...
d17_ord$bundestag_trust <- factor(d17_ord$bundestag_trust)
d17_ord$swd <- factor(d17_ord$swd)

# Models
summary(clm(bundestag_trust ~ treat17 + post17 + treat17 * post17, data = d17_ord))
summary(clm(swd ~ treat17 + post17 + treat17 * post17, data = d17_ord))
...
# Convert outcome variables to factor
d21_ord$bundestag_trust <- factor(d21_ord$bundestag_trust)
d21_ord$swd <- factor(d21_ord$swd)

# Models
summary(clm(bundestag_trust ~ treat21 + post21 + treat21 * post21, data = d21_ord))
summary(clm(swd ~ treat21 + post21 + treat21 * post21, data = d21_ord))
...
# Convert outcome variables to factor
afd17_ord$bundestag_trust <- factor(afd17_ord$bundestag_trust)
afd17_ord$swd <- factor(afd17_ord$swd)

# Models
summary(clm(bundestag_trust ~ treat17 + post17 + treat17 * post17, data = afd17_ord))
summary(clm(swd ~ treat17 + post17 + treat17 * post17, data = afd17_ord))
...
# Convert outcome variables to factor
afd21_ord$bundestag_trust <- factor(afd21_ord$bundestag_trust)
afd21_ord$swd <- factor(afd21_ord$swd)

# Models
summary(clm(bundestag_trust ~ treat21 + post21 + treat21 * post21, data = afd21_ord))
summary(clm(swd ~ treat21 + post21 + treat21 * post21, data = afd21_ord))
...
d17_bin$bundestag_trust_binary <- ifelse(d17_bin$bundestag_trust <= 2, 0, 
                                         ifelse(d17_bin$bundestag_trust > 3, 1, NA))
d17_bin$swd_binary <- ifelse(d17_bin$swd <= 2, 0,
                             ifelse(d17_bin$swd > 3, 1, NA))

# Models
summary(lm(bundestag_trust_binary ~ treat17 + post17 + treat17 * post17, data = d17_bin))
summary(lm(swd_binary ~ treat17 + post17 + treat17 * post17, data = d17_bin))
...
# Convert outcome variable to binary
d21_bin$bundestag_trust_binary <- ifelse(d21_bin$bundestag_trust <= 2, 0, 
                                         ifelse(d21_bin$bundestag_trust > 3, 1, NA))
d21_bin$swd_binary <- ifelse(d21_bin$swd <= 2, 0,
                             ifelse(d21_bin$swd > 3, 1, NA))

# Models
summary(lm(bundestag_trust_binary ~ treat21 + post21 + treat21 * post21, data = d21_bin))
summary(lm(swd_binary ~ treat21 + post21 + treat21 * post21, data = d21_bin))
...
# Models
summary(lm(bundestag_trust ~ treat17 + post17 + treat17 * post17, data = afd17_bin))
summary(lm(swd ~ treat17 + post17 + treat17 * post17, data = afd17_bin))
...
# Models
summary(lm(bundestag_trust ~ treat21 + post21 + treat21 * post21, data = afd21_bin))
summary(lm(swd ~ treat21 + post21 + treat21 * post21, data = afd21_bin))

### INSTITUTIONAL TRUST --------------------------------------------------------

## HYPOTHESIS 0
### 2017 treatment
# Trust in the Federal Constitutional Court
summary(lm(fcc_trust ~ treat17 + post17 + treat17 * post17, data = d17))

# Trust in the army
summary(lm(army_trust ~ treat17 + post17 + treat17 * post17, data = d17))

# Trust in the police
summary(lm(police_trust ~ treat17 + post17 + treat17 * post17, data = d17))

### 2021 treatment
# Trust in the Federal Constitutional Court
summary(lm(fcc_trust ~ treat21 + post21 + treat21 * post21, data = d21))

# Trust in the army
summary(lm(army_trust ~ treat21 + post21 + treat21 * post21, data = d21))

# Trust in the police
summary(lm(police_trust ~ treat21 + post21 + treat21 * post21, data = d21))

## HYPOTHESIS 1
### 2017 treatment
# Trust in the Federal Constitutional Court
summary(lm(fcc_trust ~ treat17 + post17 + treat17 * post17, data = afd17))

# Trust in the army
summary(lm(army_trust ~ treat17 + post17 + treat17 * post17, data = afd17))

# Trust in the police
summary(lm(police_trust ~ treat17 + post17 + treat17 * post17, data = afd17))

### 2021 treatment
# Trust in the Federal Constitutional Court
summary(lm(fcc_trust ~ treat21 + post21 + treat21 * post21, data = afd21))

# Trust in the army
summary(lm(army_trust ~ treat21 + post21 + treat21 * post21, data = afd21))

# Trust in the police
summary(lm(police_trust ~ treat21 + post21 + treat21 * post21, data = afd21))

### LONG-TERM OUTCOME VARIABLES ------------------------------------------------

## HYPOTHESIS 0
# Create vector of wave numbers
wave_numbers <- c(9, 10, 12, 14, 15)

# Loop through each wave number
for (n in wave_numbers) {
  # Filter data for the current wave number
  d17_long <- d[(d$wave == 2 | d$wave == 5 | d$wave == n),]
  
  # Eliminate respondents with non-responses in constituency variable
  d17_long <- d17_long[d17_long$const17 > 0, ]
  
  # Create treatment variable
  d17_long$treat17 <- ifelse(d17_long$const17 %in% c(156:158), 1, 0)
  
  # Create post variable
  d17_long$post17 <- ifelse(d17_long$wave == n, 1, 0)
  
  # Run DID model for political trust
  pt_model <- lm(bundestag_trust ~ treat17 + post17 + treat17 * post17, data = d17_long)
  
  # Run DID model for satisfaction with democracy
  swd_model <- lm(swd ~ treat17 + post17 + treat17 * post17, data = d17_long)
  
  # Add the current wave results to the overall results data frame
  cat('Wave', n, '- Political trust:')
  print(summary(pt_model))
  cat('Wave', n, '- Satisfaction with democracy:')
  print(summary(swd_model))
}
...
for (n in wave_numbers) {
  # Filter data for the current wave number
  afd17_long <- d[(d$wave == 2 | d$wave == 5 | d$wave == n),]
  
  # Eliminate respondents with non-responses in constituency variable
  afd17_long <- afd17_long[afd17_long$const17 > 0, ]
  
  # Subset to AfD voters
  afd17_long <- afd17_long %>% 
    group_by(lfdn) %>% 
    filter(any(first_vote_a == 322 | first_vote_b == 322)) %>% 
    ungroup()
  
  # Create treatment variable
  afd17_long$treat17 <- ifelse(afd17_long$const17 %in% c(156:158), 1, 0)
  
  # Create post variable
  afd17_long$post17 <- ifelse(afd17_long$wave == n, 1, 0)
  
  # Run DID model for political trust
  pt_model <- lm(bundestag_trust ~ treat17 + post17 + treat17 * post17, data = afd17_long)
  
  # Run DID model for satisfaction with democracy
  swd_model <- lm(swd ~ treat17 + post17 + treat17 * post17, data = afd17_long)
