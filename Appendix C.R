### APPENDIX D - Testing the parallel trends assumption ###
...
# Clean environment
rm(list = setdiff(ls(), c('d', 'd_raw', 'pop_den17', 'pop_den21', 'foreign17', 'foreign21', 'unemp17', 'unemp21', 'afd_results', 'g17', 'g21')))
...
### PLACEBO DIFFERENCE-IN-DIFFERENCE (H0) --------------------------------------
...
# Convert wave variable to numeric
d_plac_pt$wave <- as.numeric(d_plac_pt$wave)

# Placebo on wave 16 (period 0)
d_plac_pt <- d_plac_pt[d_plac_pt$wave <= 16,]
d_plac_pt$post <- ifelse(d_plac_pt$wave == 16, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = d_plac_pt))

# Placebo on wave 15 (period -1)
d_plac_pt <- d_plac_pt[d_plac_pt$wave <= 15,]
d_plac_pt$post <- ifelse(d_plac_pt$wave == 15, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = d_plac_pt)) 

# Placebo on wave 14 (period -2)
d_plac_pt <- d_plac_pt[d_plac_pt$wave <= 14,]
d_plac_pt$post <- ifelse(d_plac_pt$wave == 14, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = d_plac_pt)) 

# Placebo on wave 12 (period -3)
d_plac_pt <- d_plac_pt[d_plac_pt$wave <= 12,]
d_plac_pt$post <- ifelse(d_plac_pt$wave == 12, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = d_plac_pt))

# Placebo on wave 10 (period -4)
d_plac_pt <- d_plac_pt[d_plac_pt$wave <= 10,]
d_plac_pt$post <- ifelse(d_plac_pt$wave == 10, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = d_plac_pt)) 

# Placebo on wave 9 (period -5)
d_plac_pt <- d_plac_pt[d_plac_pt$wave <= 9,]
d_plac_pt$post <- ifelse(d_plac_pt$wave == 9, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = d_plac_pt))

# Placebo on wave 8 (period -6)
d_plac_pt <- d_plac_pt[d_plac_pt$wave <= 8,]
d_plac_pt$post <- ifelse(d_plac_pt$wave == 8, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = d_plac_pt))
...
# Convert wave variable to numeric
d_plac_swd$wave <- as.numeric(d_plac_swd$wave)

# Placebo on wave 16 (period 0)
d_plac_swd <- d_plac_swd[d_plac_swd$wave <= 16,]
d_plac_swd$post <- ifelse(d_plac_swd$wave == 16, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = d_plac_swd))

# Placebo on wave 15 (period -1)
d_plac_swd <- d_plac_swd[d_plac_swd$wave <= 15,]
d_plac_swd$post <- ifelse(d_plac_swd$wave == 15, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = d_plac_swd))

# Placebo on wave 14 (period -2)
d_plac_swd <- d_plac_swd[d_plac_swd$wave <= 14,]
d_plac_swd$post <- ifelse(d_plac_swd$wave == 14, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = d_plac_swd))

# Placebo on wave 12 (period -3)
d_plac_swd <- d_plac_swd[d_plac_swd$wave <= 12,]
d_plac_swd$post <- ifelse(d_plac_swd$wave == 12, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = d_plac_swd)) 

# Placebo on wave 10 (period -4)
d_plac_swd <- d_plac_swd[d_plac_swd$wave <= 10,]
d_plac_swd$post <- ifelse(d_plac_swd$wave == 10, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = d_plac_swd))

# Placebo on wave 9 (period -5)
d_plac_swd <- d_plac_swd[d_plac_swd$wave <= 9,]
d_plac_swd$post <- ifelse(d_plac_swd$wave == 9, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = d_plac_swd))

# Placebo on wave 8 (period -6)
d_plac_swd <- d_plac_swd[d_plac_swd$wave <= 8,]
d_plac_swd$post <- ifelse(d_plac_swd$wave == 8, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = d_plac_swd))

### PLACEBO DIFFERENCE-IN-DIFFERENCE (H1) --------------------------------------
...
# Eliminate respondents with non-responses in constituency variable
afd_plac_pt <- afd_plac_pt[afd_plac_pt$const21 > 0, ]

# Eliminate respondents from constituencies where AfD did not field candidates
afd_plac_pt <- afd_plac_pt %>% filter(!(const21 %in% c(24, 34, 53, 140, 223, 251)))

# Eliminate respondents from pre-treated constituencies
afd_plac_pt <- afd_plac_pt %>% filter(!(const21 %in% c(156, 157, 158)))

# Subset to waves of interest
afd_plac_pt <- afd_plac_pt %>% filter((wave %in% c(2, 8, 9, 10, 12, 14, 15, 16, 20)))

# Subset to AfD respondents
afd_plac_pt <- afd_plac_pt %>% 
  group_by(lfdn) %>% 
  filter(any(first_vote_a == 322 | first_vote_b == 322)) %>% 
  ungroup()

# Create treatment variable 
afd_plac_pt$treat21 <- ifelse(afd_plac_pt$const21 %in% c(71, 74, 151, 154, 155, 161, 163:165, 190, 192, 194, 195), 1, 0)

# Convert wave variable to numeric
afd_plac_pt$wave <- as.numeric(afd_plac_pt$wave)

# Placebo on wave 16 (period 0)
afd_plac_pt <- afd_plac_pt[afd_plac_pt$wave <= 16,]
afd_plac_pt$post <- ifelse(afd_plac_pt$wave == 16, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = afd_plac_pt)) 

# Placebo on wave 15 (period -1)
afd_plac_pt <- afd_plac_pt[afd_plac_pt$wave <= 15,]
afd_plac_pt$post <- ifelse(afd_plac_pt$wave == 15, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = afd_plac_pt)) 

# Placebo on wave 14 (period -2)
afd_plac_pt <- afd_plac_pt[afd_plac_pt$wave <= 14,]
afd_plac_pt$post <- ifelse(afd_plac_pt$wave == 14, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = afd_plac_pt))

# Placebo on wave 12 (period -3)
afd_plac_pt <- afd_plac_pt[afd_plac_pt$wave <= 12,]
afd_plac_pt$post <- ifelse(afd_plac_pt$wave == 12, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = afd_plac_pt))

# Placebo on wave 10 (period -4)
afd_plac_pt <- afd_plac_pt[afd_plac_pt$wave <= 10,]
afd_plac_pt$post <- ifelse(afd_plac_pt$wave == 10, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = afd_plac_pt))

# Placebo on wave 9 (period -5)
afd_plac_pt <- afd_plac_pt[afd_plac_pt$wave <= 9,]
afd_plac_pt$post <- ifelse(afd_plac_pt$wave == 9, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = afd_plac_pt))

# Placebo on wave 8 (period -6)
afd_plac_pt <- afd_plac_pt[afd_plac_pt$wave <= 8,]
afd_plac_pt$post <- ifelse(afd_plac_pt$wave == 8, 1, 0)
summary(lm(bundestag_trust ~ treat21 + post + treat21*post, data = afd_plac_pt))
...
# Eliminate respondents with non-responses in constituency variable
afd_plac_swd <- afd_plac_swd[afd_plac_swd$const21 > 0, ]

# Eliminate respondents from constituencies where AfD did not field candidates
afd_plac_swd <- afd_plac_swd %>% filter(!(const21 %in% c(24, 34, 53, 140, 223, 251)))

# Eliminate respondents from pre-treated constituencies
afd_plac_swd <- afd_plac_swd %>% filter(!(const21 %in% c(156, 157, 158)))

# Subset to waves of interest
afd_plac_swd <- afd_plac_swd %>% filter((wave %in% c(5, 8, 9, 10, 12, 14, 15, 16, 20)))

# Subset to AfD respondents
afd_plac_swd <- afd_plac_swd %>% 
  group_by(lfdn) %>% 
  filter(any(first_vote_a == 322 | first_vote_b == 322)) %>% 
  ungroup()

# Create treatment variable 
afd_plac_swd$treat21 <- ifelse(afd_plac_swd$const21 %in% c(71, 74, 151, 154, 155, 161, 163:165, 190, 192, 194, 195), 1, 0)

# Convert wave variable to numeric
afd_plac_swd$wave <- as.numeric(afd_plac_swd$wave)

# Placebo on wave 16 (period 0)
afd_plac_swd <- afd_plac_swd[afd_plac_swd$wave <= 16,]
afd_plac_swd$post <- ifelse(afd_plac_swd$wave == 16, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = afd_plac_swd)) 

# Placebo on wave 15 (period -1)
afd_plac_swd <- afd_plac_swd[afd_plac_swd$wave <= 15,]
afd_plac_swd$post <- ifelse(afd_plac_swd$wave == 15, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = afd_plac_swd)) 

# Placebo on wave 14 (period -2)
afd_plac_swd <- afd_plac_swd[afd_plac_swd$wave <= 14,]
afd_plac_swd$post <- ifelse(afd_plac_swd$wave == 14, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = afd_plac_swd)) 

# Placebo on wave 12 (period -3)
afd_plac_swd <- afd_plac_swd[afd_plac_swd$wave <= 12,]
afd_plac_swd$post <- ifelse(afd_plac_swd$wave == 12, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = afd_plac_swd))

# Placebo on wave 10 (period -4)
afd_plac_swd <- afd_plac_swd[afd_plac_swd$wave <= 10,]
afd_plac_swd$post <- ifelse(afd_plac_swd$wave == 10, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = afd_plac_swd))

# Placebo on wave 9 (period -5)
afd_plac_swd <- afd_plac_swd[afd_plac_swd$wave <= 9,]
afd_plac_swd$post <- ifelse(afd_plac_swd$wave == 9, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = afd_plac_swd))

# Placebo on wave 8 (period -6)
afd_plac_swd <- afd_plac_swd[afd_plac_swd$wave <= 8,]
afd_plac_swd$post <- ifelse(afd_plac_swd$wave == 8, 1, 0)
summary(lm(swd ~ treat21 + post + treat21*post, data = afd_plac_swd))